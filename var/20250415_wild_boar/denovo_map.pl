...
use File::Spec;
use constant stacks_version => "2.68";

use constant true  => 1;
use constant false => 0;

my $dry_run      = false;
my $exe_path     = "/usr/local/stacks-2.68/bin/";
my $out_path     = "";
my $cpopmap_path = "";
my $popmap_path  = "";
my $sample_path  = "";
...
sub check_return_value {
    ...
    if ($rv != 0) {
        ...
        $msg .= ")";
        print $log_fh ($msg . ".\n");
        print $log_fh "Last command executed by denovo_map.pl was:\n  $last_cmd\n";
        print STDERR ($msg . "; see denovo_map.log file.\n");
        print STDERR "Last command executed by denovo_map.pl was:\n  $last_cmd\n";
        exit 1;
    }
}
